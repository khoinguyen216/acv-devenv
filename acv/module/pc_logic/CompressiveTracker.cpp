...
//------------------------------------------------
CompressiveTracker::CompressiveTracker(int nFeatures, int r)
{
	featureMinNumRect = 2;
	featureMaxNumRect = 4;	// number of rectangle from 2 to 4
	featureNum = nFeatures;	// number of all weaker classifiers, i.e,feature pool
	rOuterPositive = 4;	// radical scope of positive samples
	rSearchWindow = r; // size of search window
	muPositive = vector<float>(featureNum, 0.0f);
	muNegative = vector<float>(featureNum, 0.0f);
	sigmaPositive = vector<float>(featureNum, 1.0f);
	sigmaNegative = vector<float>(featureNum, 1.0f);
...
CompressiveTracker::CompressiveTracker(const CompressiveTracker& ct)
{
	featureMinNumRect = ct.featureMinNumRect;
	featureMaxNumRect = ct.featureMaxNumRect; 
	featureNum = ct.featureNum;
	features = ct.features;
	featuresWeight = ct.featuresWeight;
	rOuterPositive = ct.rOuterPositive;
	samplePositiveBox = ct.samplePositiveBox;
	sampleNegativeBox = ct.sampleNegativeBox;
	rSearchWindow = ct.rSearchWindow;
	imageIntegral = ct.imageIntegral.clone();
	samplePositiveFeatureValue = ct.samplePositiveFeatureValue.clone();
	sampleNegativeFeatureValue = ct.sampleNegativeFeatureValue.clone();
	muPositive = ct.muPositive;
	sigmaPositive = ct.sigmaPositive;
	muNegative = ct.muNegative;
	sigmaNegative = ct.sigmaNegative;
	learnRate = ct.learnRate;
	detectBox = ct.detectBox;
	detectFeatureValue = ct.detectFeatureValue.clone();
	rng = RNG(ct.rng);
}
...
void CompressiveTracker::HaarFeature(const Rect& _objectBox, int _numFeature)
...
void CompressiveTracker::sampleRect(const Mat& _image, const Rect& _objectBox, float _rInner, float _rOuter, int _maxSampleNum, vector<Rect>& _sampleBox)
...
*/
{
	...
	////// Border check
	//minrow = max(0, min(_image.rows-1, minrow));
	//maxrow = max(0, min(_image.rows-1, maxrow));
	//mincol = max(0, min(_image.cols, mincol));
	//maxcol = max(0, min(_image.cols, maxcol));
...
void CompressiveTracker::sampleRect(const Mat& _image, const Rect& _objectBox, float _srw, vector<Rect>& _sampleBox)
/* Description: Compute the coordinate of samples when detecting the object.*/
{
	...
	minrow = max(0, min(_image.rows-1, minrow));
	maxrow = max(0, min(_image.rows-1, maxrow));
	mincol = max(0, min(_image.cols-1, mincol));
	maxcol = max(0, min(_image.cols-1, maxcol));
...
void CompressiveTracker::getFeatureValue(const Mat& _imageIntegral, vector<Rect>& _sampleBox, Mat& _sampleFeatureValue)
{
	...
	for (int i=0; i<featureNum; i++)
	{
		for (int j=0; j<sampleBoxSize; j++)
		{
			...
			for (size_t k=0; k<features[i].size(); k++)
			{
				...
				xMin = min(max(0, xMin), _imageIntegral.cols-1);
				xMax = min(max(0, xMax), _imageIntegral.cols-1);
				yMin = min(max(0, yMin), _imageIntegral.rows-1);
				yMax = min(max(0, yMax), _imageIntegral.rows-1);
...
void CompressiveTracker::classifierUpdate(const Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate)
{
	Scalar muTemp;
	Scalar sigmaTemp;
...
void CompressiveTracker::init(const Mat& _frame, const Rect& _objectBox)
{
...
void CompressiveTracker::processFrame(const Mat& _frame, Rect& _objectBox)
{
	// predict
	sampleRect(_frame, _objectBox, rSearchWindow,detectBox);
	if (detectBox.size() <= 0) return;
	...
	if (samplePositiveBox.size() <= 0) return;
	sampleRect(_frame, _objectBox, rSearchWindow*1.5, rOuterPositive+4.0, 100, sampleNegativeBox);
	if (sampleNegativeBox.size() <= 0) return;
