#ifndef _BLOCKLBPDETECTOR_WITH_POSE_H_
#define _BLOCKLBPDETECTOR_WITH_POSE_H_
/*
...
class BlockLBPDetector_pose:public LBPDetector
{
private:
	/**
	 * Disable the default empty constructor since I still don't need it
	 */
	LBPDETECTOR_API BlockLBPDetector_pose(void);
	/**
	 * Disable the default copy constructor since I still don't need it
	 */
	LBPDETECTOR_API BlockLBPDetector_pose(const BlockLBPDetector_pose&);
	/**
	 * Disable the default assignment operator since I still don't need it
	 */
	LBPDETECTOR_API BlockLBPDetector_pose& operator = (const BlockLBPDetector_pose&);

public:
	/**
	 * Constructor
	 */
	LBPDETECTOR_API BlockLBPDetector_pose(int width, int height, int blockWidth, int blockHeight,
	 ...
	 */
	LBPDETECTOR_API BlockLBPDetector_pose(const DLBPConfiguration& config)
		:LBPDetector(config)
	{
		detector.clear();
		recognizer.clear();
	...
	/*
	 * Load the pose classifiers. To tell which pose this image patch is in.
	 */
	LBPDETECTOR_API void LoadPoseClassifiers(vector<string> filepaths);

	/*
	 * Load the detector for each pose
	 */
	LBPDETECTOR_API void LoadPoseDetectors(vector<string> filepaths);
	...
	LBPDETECTOR_API bool DetectObjects(const IplImage *image, std::vector<CvRect>& resultRect, std::vector<Orientation> *orientation = 0,
		float threshold = 0.0f, int groupThreshold = 2, float scaleStep = 1.1, const vector<cv::Rect> *maskRects = 0);
...
private:
	 ...
	 */
	LBPDETECTOR_API bool FindAllCandidates(const IplImage *image, float threshold = 0.0, float scaleStep = 1.1, const vector<cv::Rect> *maskRects = 0);

	LBPDETECTOR_API bool FindAllCandidates_aspectratio(const IplImage *image, float threshold, float scaleStep, const vector<cv::Rect> *maskRects = 0);
...
private:
		vector<vector<float>> m_detectors, m_classifiers;
		vector<float> m_rho_detectors, m_rgo_classifiers;
