...
public:
	CompressiveTracker();
	CompressiveTracker(int nFeatures, int r);
	CompressiveTracker(const CompressiveTracker& ct);
...
private:
	void HaarFeature(const Rect& _objectBox, int _numFeature);
	void sampleRect(const Mat& _image, const Rect& _objectBox, float _rInner, float _rOuter, int _maxSampleNum, vector<Rect>& _sampleBox);
	void sampleRect(const Mat& _image, const Rect& _objectBox, float _srw, vector<Rect>& _sampleBox);
	void getFeatureValue(const Mat& _imageIntegral, vector<Rect>& _sampleBox, Mat& _sampleFeatureValue);
	void classifierUpdate(const Mat& _sampleFeatureValue, vector<float>& _mu, vector<float>& _sigma, float _learnRate);
	void radioClassifier(vector<float>& _muPos, vector<float>& _sigmaPos, vector<float>& _muNeg, vector<float>& _sigmaNeg,
						Mat& _sampleFeatureValue, float& _radioMax, int& _radioMaxIndex);
public:
	void processFrame(const Mat& _frame, Rect& _objectBox);
	void init(const Mat& _frame, const Rect& _objectBox);
};
