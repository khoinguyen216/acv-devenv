...
bgfg_vibe::bgfg_vibe():R(20),N(20),noMin(2),phi(0),nochs(0)
{
	initDone=false;
	rnd = cv::RNG(23423423);
	ri=0;
...
void bgfg_vibe::rndreset()
{
	for(int ch=0;ch<nochs;ch++)
	{
		cv::Mat** samples = model->samples[ch];

		int step=samples[0]->step.p[0];

		for(int i=0;i<samples[0]->rows;i++)
		{
			int ioff= step*i;
			for(int j=0;j<samples[0]->cols;j++)
			{
				int val =rnd(255);
				int rand = rndn[rdx];
				if((model->fgch[ch]->data+ioff)[j]==255)
				{
					(samples[rand]->data + ioff)[j]=val;
				}
			}
		}
	}
...
cv::Mat bgfg_vibe::bg()
{
	...
	if(nochs==1)
	{
		...
		ret= cv::Mat::zeros(model->samples[0][0]->size(),CV_8UC1);
		chs.push_back(ret);
	}
	if(nochs==3)
	{
		ret= cv::Mat::zeros(model->samples[0][0]->size(),CV_8UC3);
		split(ret,chs);
	}
